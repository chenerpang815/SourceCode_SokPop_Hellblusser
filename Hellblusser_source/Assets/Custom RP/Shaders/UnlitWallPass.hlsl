...
Varyings UnlitWallPassVertex (Attributes input)
{
	Varyings output;
...
float4 UnlitWallPassFragment (Varyings input) : SV_TARGET
{
	UNITY_SETUP_INSTANCE_ID(input);
	...
	float c0 = .0075;
	float h = sin(((input.positionWS.y) * 10));
	if ( h < .9 || input.positionWS.y < 1 )
	{
