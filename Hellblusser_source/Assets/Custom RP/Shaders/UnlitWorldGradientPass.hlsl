...
Varyings UnlitWorldGradientPassVertex (Attributes input)
{
	Varyings output;
...
float4 UnlitWorldGradientPassFragment (Varyings input) : SV_TARGET
{
	UNITY_SETUP_INSTANCE_ID(input);
	...
	float h = (hMax - input.positionWS.y) / (hMax - hMin);
