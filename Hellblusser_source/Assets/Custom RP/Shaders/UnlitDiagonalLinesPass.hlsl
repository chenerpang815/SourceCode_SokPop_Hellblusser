...
Varyings UnlitDiagonalLinesPassVertex (Attributes input)
{
	Varyings output;
...
float4 UnlitDiagonalLinesPassFragment (Varyings input) : SV_TARGET
{
	UNITY_SETUP_INSTANCE_ID(input);
	...
	float c0 = .0075;
	c += abs(sin(((input.positionCS_SS.x - input.positionCS_SS.y) * .5) * 1.5)) * c0;
