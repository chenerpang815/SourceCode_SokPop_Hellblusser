...
Varyings UnlitVerticalLinesPass2Vertex (Attributes input)
{
	Varyings output;
...
float4 UnlitVerticalLinesPass2Fragment (Varyings input) : SV_TARGET
{
	UNITY_SETUP_INSTANCE_ID(input);
	...
	if ( input.positionWS.y < .15 )
	{
		float c1 = GetBottomAdd(config);
		c.rgb += float3(c1,c1,c1);
	}
	else
	{
		float c0 = GetHighlightAdd(config); //.025;
		float h = sin(((input.positionWS.x + input.positionWS.z) * 8));
		if ( h < 0 )
		{
