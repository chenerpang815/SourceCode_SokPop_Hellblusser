...
;
            ...
            {
                float2 dir = frac(input.tex0*_TextureSizeVector.xy/2.0) - float2(0.5,0.5);
                if ((dir.x*dir.y)>0.0) return tex2D(_MainTex, input.tex0);
                float2 g1 = float2(1.0/_TextureSizeVector.xy.x, 0.0);
                float2 g2 = float2(0.0, 1.0/_TextureSizeVector.xy.y);
                float3 P0 = tex2D(_MainTex, input.tex0 -3.0*g1       ).xyz;
                float3 P1 = tex2D(_MainTex, input.tex0        -3.0*g2).xyz;
                float3 P2 = tex2D(_MainTex, input.tex0        +3.0*g2).xyz;
                float3 P3 = tex2D(_MainTex, input.tex0 +3.0*g1       ).xyz;
                float3 B = tex2D(_MainTex, input.tex0 -2.0*g1     -g2).xyz;
                float3 C = tex2D(_MainTex, input.tex0     -g1 -2.0*g2).xyz;
                float3 D = tex2D(_MainTex, input.tex0 -2.0*g1     +g2).xyz;
                float3 E = tex2D(_MainTex, input.tex0     -g1        ).xyz;
                float3 F = tex2D(_MainTex, input.tex0             -g2).xyz;
                float3 G = tex2D(_MainTex, input.tex0     -g1 +2.0*g2).xyz;
                float3 H = tex2D(_MainTex, input.tex0             +g2).xyz;
                float3 I = tex2D(_MainTex, input.tex0     +g1        ).xyz;
                float3 F4 = tex2D(_MainTex,input.tex0     +g1 -2.0*g2).xyz;
                float3 I4 = tex2D(_MainTex,input.tex0 +2.0*g1     -g2).xyz;
                float3 H5 = tex2D(_MainTex,input.tex0     +g1 +2.0*g2).xyz;
                float3 I5 = tex2D(_MainTex,input.tex0 +2.0*g1     +g2).xyz;
                float b = RGBtoYUV( B );
                float c = RGBtoYUV( C );
                float d = RGBtoYUV( D );
                float e = RGBtoYUV( E );
